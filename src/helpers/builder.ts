...
function addInfoItem(info: Element, type: InfoType, text?: string | null, isVideo = true) {
    ...
    if (item) {
        const info = item.querySelector(".num-info");
        if (info instanceof HTMLElement) { info.textContent = text!; }
        return;
    }
...
function addInfoItems(info: Element, types?: readonly InfoType[] | null, card?: IBiliBiliCard) {
...
export function getDefaultInfoTypes(value: CardType) {
    switch (value) {
        case "video":
            return ["views", "danmakus"] as const;
        case "user":
            return ["views", "likes"] as const;
        case "live":
            return ["views"] as const;
        case "bangumi":
            return ["favorites"] as const;
        case "favorite":
            return ["views", "favorites"] as const;
        case "article":
        case "audio":
        case "dynamic":
        case "album":
        default:
            return ["views", "comments"] as const;
    }
}
...
export function connectedCallback(this: IBiliBiliCard) {
...
export function attributeChangedCallback(this: IBiliBiliCard, name: string, newValue: string) {
    switch (name) {
        case "vid":
        ...
        case "info-types":
            ...
            info.innerHTML = '';
            let types = typeof newValue === "string" ? newValue.split(/[,|\s+]/).filter(x => x != '') as readonly InfoType[] : [];
            if (!types.length) {
